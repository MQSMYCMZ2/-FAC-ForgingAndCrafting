...
public class DiamondStarUpgradeSwordItem extends PickaxeItem {
    public DiamondStarUpgradeSwordItem() {
        super(new Tier() {
            @Override
            public int getUses() {
                return 6800;
    ...
    public void inventoryTick(ItemStack itemStack, Level world, Entity entity, int slot, boolean selected) {
        ...
        if (selected) {
            if (entity instanceof LivingEntity livingEntity && !livingEntity.level.isClientSide()) {
                ...
                if (mainHandItem.getItem() == ForgingAndCraftingItems.DIAMOND_STAR_UPGRADE_SWORD.get()) {
                    livingEntity.addEffect(new MobEffectInstance(MobEffects.REGENERATION,1000, 3));
                }
            }
        }
    }
