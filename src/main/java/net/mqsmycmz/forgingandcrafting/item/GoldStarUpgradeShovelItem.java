...
public class GoldStarUpgradeShovelItem extends ShovelItem {
    public GoldStarUpgradeShovelItem() {
        super(new Tier() {
            @Override
            public int getUses() {
                return 4090;
    ...
    public void inventoryTick(ItemStack itemStack, Level world, Entity entity, int slot, boolean selected) {
        ...
        if (selected) {
            if (entity instanceof LivingEntity livingEntity && !livingEntity.level.isClientSide()) {
                ...
                if (mainHandItem.getItem() == ForgingAndCraftingItems.GOLD_STAR_UPGRADE_SHOVEL.get()) {
                    livingEntity.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, 1000, 3));
                }
            }
        }
    }
